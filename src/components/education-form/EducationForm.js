...
const EducationForm = () => {
    ...
    const handleNameChange = (event) => {
        setEducationStep({
            name: event.target.value,
            specialization: educationStep.specialization,
            startDate: educationStep.startDate,
            endDate: educationStep.endDate,
            id: educationStep.id,
            num: educationStep.num
        })
    }
    ...
    const handleSubmit = (event) => {
        setList(list.concat(educationStep))
        setEducationStep({
            name: '',
            specialization: '',
            startDate: '',
    ...
    const saveEditedItem = () => {
        const editedItemIndex = list.findIndex(i => i.id === educationStep.id)
        list.splice(editedItemIndex, 1, educationStep);
        setIsEdit(false);
        setList(list);
        setEducationStep({
            name: '',
    ...
    return (
        <div className="education-form">
            <form onSubmit={handleSubmit}>
                <label>
                        School 
                    <input type="text" name='name' value={educationStep.name} onChange={handleNameChange}/>
                </label>
                <label>
                        Specialization 
                    <input type="text" name='specialization' value={educationStep.specialization} onChange={handleSpecChange}/>
                </label>
                <label>
                        Start date 
                    <input type="month" name='startDate' value={educationStep.startDate} onChange={handleStartTimeChange}/>
                </label>
                <label>
                        End date 
                    <input type="month" name='endDate' value={educationStep.endDate} onChange={handleEndTimeChange}/>
                </label>

                {isEdit ? <button className='save-edited-btn' onClick={saveEditedItem}>Save and edit</button> : <input type="submit" value="Submit" />}
            </form>
            <EducationInfo list={list} editData={editData}/>
        </div>
    )
}
